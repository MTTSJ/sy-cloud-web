        ...
        >
            ...
            <template slot="javaType" slot-scope="text, record">
                <a-select style="width: 120px" v-model="record.javaType" :disabled="judgeColumns(record)">
                    <a-select-option v-for="(item, index) in javaTypeData" :key="index" :value="item.dictKey">
                        {{ item.dictValue }}
                    </a-select-option>
                </a-select>
            </template>
            <template slot="effectType" slot-scope="text, record">
                <a-select style="width: 120px" v-model="record.effectType" :disabled="judgeColumns(record)">
                    <a-select-option v-for="(item, index) in effectTypeData" :key="index" :value="item.dictKey">
                        {{ item.dictValue }}
                    </a-select-option>
                </a-select>
            </template>
            <template slot="dictTypeCode" slot-scope="text, record">
                <a-select
                    ...
                    "
                >
                    <a-select-option v-for="(item, index) in dictDataAll" :key="index" :value="item.id">
                        {{ item.dictValue }}
                    </a-select-option>
                </a-select>
            </template>
            ...
            <template slot="queryType" slot-scope="text, record">
                <a-select style="width: 100px" v-model="record.queryType" :disabled="!record.queryWhether">
                    <a-select-option v-for="(item, index) in codeGenQueryTypeData" :key="index" :value="item.dictKey">
                        {{ item.dictValue }}
                    </a-select-option>
                </a-select>
            </template>
    ...
    export default {
        ...
        methods: {
            ...
            open(data) {
                ...
                this.javaTypeData = dictOption.filters['dictData']('codeJavaType');
                this.effectTypeData = dictOption.filters['dictData']('codeEffectType');
                this.dictDataAll = dictOption.filters['dictDataAll']();
                this.yesOrNoData = dictOption.filters['dictData']('ok');
                this.codeGenQueryTypeData = dictOption.filters['dictData']('codeQueryType');
                const params = {
                    codeId: data.id,
                };
            ...
            judgeColumns(data) {
                if (
                    ...
                    data.columnName.indexOf('create_by') > -1 ||
                    data.columnName.indexOf('update_by') > -1 ||
