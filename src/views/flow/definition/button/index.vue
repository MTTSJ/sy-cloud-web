        ...
        <div v-if="hasPerm('defin:page')">
            ...
            >
                <span slot="button_yn" slot-scope="text, record">
                    ...
                    <span v-else>{{ okFilter(text) }}</span>
                </span>
                            ...
                            <div>
                                <div :style="{ borderBottom: '1px solid #E9E9E9' }">
                                    <a-checkbox
                                        ...
                                        :checked="addCheckAll"
                                        @change="addOnCheckAllChange"
                                    >
                                        全选
                                    </a-checkbox>
                                </div>
                                <a-checkbox-group
                                    v-model="addCheckedList"
                                    @change="addOnChange"
                                >
                                    <a-row>
                                        <a-col
                                            style="padding-top: 5px; padding-bottom: 5px"
                                            v-for="(item, index) in columnsButtonTitle"
                                            :key="index"
                                            :span="6"
                                        >
                                            <a-checkbox :checked="true" :value="item.dataIndex">
                                                {{ item.title }}
                                            </a-checkbox>
                                        </a-col>
                                    </a-row>
...
<script>
    import {
        ...
        buttonEdit,
        buttonDelete,
    } from '@/api/modular/flowable/buttonManage';
    ...
    export default {
        ...
        methods: {
            ...
            addAction() {
                ...
                this.columnsButtonTitle.push(
                    {
                        title: '提交',
                        dataIndex: 'submitBtn',
                        scopedSlots: { customRender: 'button_yn' },
                    },
                    {
                        title: '保存',
                        dataIndex: 'saveBtn',
                    ...
                    {
                        title: '加签',
                        dataIndex: 'addSignBtn',
                        scopedSlots: { customRender: 'button_yn' },
                    },
                    {
                        title: '减签',
                        dataIndex: 'deleteSignBtn',
                        scopedSlots: { customRender: 'button_yn' },
                    },
                );
                ...
                if (this.hasPerm('defin:edit') || this.hasPerm('defin:del')) {
                    this.columns.push({
                        title: '操作',
                        width: '150px',
                        dataIndex: 'action',
            ...
            handleAdd() {
                ...
                setTimeout(() => {
                    this.addForm.setFieldsValue({
                        processDefinitionId: this.visibleButton.id,
                    });
                }, 100);
            },
            /**
            ...
            handleEdit(record) {
                ...
                this.addIndeterminate =
                    !!this.addCheckedList.length &&
                    this.addCheckedList.length < addPlainOptions.length;
             ...
             */
            actSelect(code) {
                this.addForm.getFieldDecorator('actName', {
                    initialValue: this.UserTasks.find((item) => code === item.id).name,
                });
            },
            /**
             * 多选框点击
            ...
            addOnChange(checkedList) {
                this.addIndeterminate =
                    !!checkedList.length && checkedList.length < addPlainOptions.length;
            ...
            handleDel(record) {
                this.tableLoading = true;
                buttonDelete({ ids: record.id }).then((res) => {
                    this.tableLoading = false;
                    if (res.success) {
