<template>
    <div>
        <div class="table-operator" v-if="hasPerm('defin:add')">
            <a-button @click="handleAdd" v-if="hasPerm('defin:add')" type="primary" icon="plus" >新增事件</a-button>
        </div>
        <div v-if="hasPerm('defin:page')">
            ...
            >
                ...
                <span slot="action" slot-scope="text,record">
                    ...
                    <a-divider type="vertical" v-if="hasPerm('defin:page') & hasPerm('defin:edit')"/>
                    <a @click="handleEdit(record)" v-if="hasPerm('defin:edit')">编辑</a>
                    <a-divider type="vertical" v-if="hasPerm('defin:del') & hasPerm('defin:edit')"/>
                    <a-popconfirm v-if="hasPerm('defin:del')" placement="topRight" title="确定删除？" @confirm="() => handleDelete(record)">
                        <a>删除</a>
                    </a-popconfirm>
                </span>
            </a-table>
                    ...
                    <a-form :form="editForm">
                        ...
                        >
                            <a-radio-group v-decorator="['nodeType',{rules: [{ required: true, message: '请选择事件节点类型！' }]}]" >
                                <a-radio v-for="(item,index) in nodeTypeData" :key="index" :value="item.dictKey" @click="nodeTypeClick(item.dictKey,'edit')">{{ item.dictValue }}</a-radio>
                            </a-radio-group>
                        </a-form-item>
                        <a-form-item
                            v-show="actIdShow"
                        ...
                        >
                            <a-select placeholder="请选择事件类型" v-decorator="['type', {rules: [{required: true, message: '请选择事件类型！'}]}]">
                                <a-select-option v-for="(item,index) in typeData" :key="index" :value="item.dictKey" >{{ item.dictValue }}</a-select-option>
                            </a-select>
                        </a-form-item>
                        <a-form-item
                            label="脚本"
            ...
            >
                <a-spin :spinning="Loading">
                    <a-form :form="addForm">
                        ...
                        >
                            <a-radio-group v-decorator="['nodeType',{rules: [{ required: true, message: '请选择事件节点类型！' }]}]" >
                                <a-radio v-for="(item,index) in nodeTypeData" :key="index" :value="item.dictKey" @click="nodeTypeClick(item.dictKey,'add')">{{ item.dictValue }}</a-radio>
                            </a-radio-group>
                        </a-form-item>
                        <a-form-item
                            v-show="actIdShow"
    ...
    export default {
        ...
        methods: {
            ...
            addAction() {
                ...
                if (this.hasPerm('defin:page') || this.hasPerm('defin:edit') || this.hasPerm('defin:del')) {
                    this.columns.push({
                        title: '操作',
                        width: '150px',
                        dataIndex: 'action',
             ...
             */
            getDictData() {
                this.nodeTypeData = this.$options.filters['dictData']('eventNodeType');
                this.typeData = this.$options.filters['dictData']('eventType');
            },
            /**
             * 事件节点类型翻译
             */
            ...
            typeFilter(text) {
                // eslint-disable-next-line eqeqeq
                const values = this.typeData.filter(item => item.dictKey == text);
                if (values.length > 0) {
                    return values[0].dictValue;
                }
            },
            ...
            handleDelete(record) {
                this.Loading = true;
                eventDelete({ ids: record.id }).then((res) => {
                    this.Loading = false;
                    if (res.success) {
