<template>
    <div>
        <x-card v-if="hasPerm('defin:page')">
                    ...
                    <a-row :gutter="48">
                        ...
                        <a-col :md="8" :sm="24">
                            <a-form-item label="定义名称">
                                <a-input v-model="queryParam.name" allow-clear placeholder="请输入定义名称"/>
                            </a-form-item>
                        </a-col>
        ...
        <a-card :bordered="false" v-show="indexPageShow">
            <a-spin :spinning="cardLoading">
                ...
                >
                    <span slot="suspended" slot-scope="text,record" v-if="hasPerm('defin:edit')">
                        <a-popconfirm placement="top" :title="text==false? '确定挂起该定义？':'确定激活该定义？'" @confirm="() => suspendedChange(text,record)">
                            <a>{{ trueFalseFilter(text) }}</a>
                        </a-popconfirm>
                    </span>
                    <span slot="suspended" v-else>
                        {{ trueFalseFilter(text) }}
                    </span>
                    <span slot="version" slot-scope="text">
                        V{{ text }}.0
                    </span>
                    <span slot="action" slot-scope="text, record">
                        <a @click="$refs.tracking.tracking(record.id)" v-if="hasPerm('defin:edit')">追踪</a>
                        <a-divider type="vertical" v-if="hasPerm('defin:edit')"/>
                        <a-popconfirm v-if="hasPerm('defin:edit')" placement="topRight" title="将该定义映射到模型设计？" @confirm="() => flowableDefinitionMapping(record)">
                            <a>映射</a>
                        </a-popconfirm>
                        ...
                        <a-dropdown>
                            ...
                            <a-menu slot="overlay">
                                ...
                                <a-menu-item v-if="hasPerm('defin:add')">
                                    <a @click="$refs.AddShortcutForm.add(record)">应用</a>
                                </a-menu-item>
                                <a-menu-item v-if="hasPerm('defin:edit')">
                                    <a @click="flowableDefinitionExport(record)">导出</a>
                                </a-menu-item>
                                <a-menu-item v-if="hasPerm('defin:del')">
                ...
                <add-shortcut-form ref="AddShortcutForm" @ok="handleOk" v-if="hasPerm('defin:add')"/>
                <tracking ref="tracking" @ok="handleOk" v-if="hasPerm('defin:edit')"/>
            </a-spin>
        </a-card>
        <div>
...
<script>
    ...
    export default {
        ...
        methods: {
             ...
             */
            dataTypeItem() {
                this.trueFalsePrefixData = this.$options.filters['dictData']('trueFalse');
            },
            trueFalseFilter(text) {
                // eslint-disable-next-line eqeqeq
                const values = this.trueFalsePrefixData.filter(item => item.dictKey == text.toString());
                if (values.length > 0) {
