    ...
    <div>
        <x-card>
            <div slot="content" class="table-page-search-wrapper">
                        ...
                        <a-col :md="8" :sm="24">
                            <a-form-item label="发起人">
                                <a-input
                                    ...
                                    allow-clear
                                    placeholder="请选择发起人"
                                    @click="$refs.AdvanceUserSelect.show()"
                                />
                            </a-form-item>
                        </a-col>
                        <a-col :md="8" :sm="24">
                        ...
                        <template v-if="advanced">
                            <a-col :md="8" :sm="24">
                                <a-form-item label="是否结束">
                                    <a-select
                                        v-model="queryParam.ended"
                                        placeholder="请选择是否结束"
                                        allow-clear
                                    >
                                        <a-select-option
                                            v-for="(item, index) in endedDictTypeDropDown"
                                            :key="index"
                                            :value="item.code"
                                        >
                                            {{ item.value }}
                ...
                <advance-user-select ref="AdvanceUserSelect" @ok="handleUser" />
            </div>
        </x-card>
                ...
                >
                    ...
                    <span slot="ended" slot-scope="text, record">
                        <div v-if="!record.ended">
                            ...
                            <div v-else>
                                <a @click="$refs.terminatedForm.terminated(record.id)">
                                    {{ endedFilter(text) }}
                                </a>
                            </div>
                        </div>
                    ...
                    <span slot="suspended" slot-scope="text, record" v-if="!record.ended">
                        <a-popconfirm
                            placement="top"
                            :title="text == false ? '确定挂起该实例？' : '确定激活该实例？'"
                            @confirm="() => suspendedChange(text, record)"
                        >
...
<script>
    import { STable, Ellipsis, XCard, AdvanceUserSelect } from '@/components';
    import {
        flowableInstancePage,
        flowableInstanceActive,
        flowableInstanceSuspend,
    } from '@/api/modular/flowable/instanceManage';
    ...
    export default {
        ...
        data() {
            return {
                ...
                loadData: (parameter) => {
                    ...
                    delete obj.dates;
                    return flowableInstancePage(Object.assign(parameter, obj)).then(
                        (res) => {
                            return res.data;
                        },
                    );
        ...
        methods: {
            ...
            suspendedFilter(suspended) {
                ...
                const values = this.suspendedDictTypeDropDown.filter(
                    (item) => item.dictKey === suspended.toString(),
                );
                if (values.length > 0) {
            ...
            endedFilter(ended) {
                // eslint-disable-next-line eqeqeq
                const values = this.endedDictTypeDropDown.filter(
                    (item) => item.dictKey === ended.toString(),
                );
                if (values.length > 0) {
                    return values[0].dictValue;
                }
             ...
             */
            sysDictTypeDropDown() {
                this.suspendedDictTypeDropDown = this.$options.filters['dictData']('suspendedStatus');
                this.endedDictTypeDropDown = this.$options.filters['dictData']('endedStatus');
            },
            /**
             * 挂起激活
             */
            ...
            handleUser(result) {
                this.$set(this.queryParam, 'startUserId', result[0].account);
            },
        },
    };
</script>
