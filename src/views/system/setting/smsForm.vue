<template>
    <a-card title="短信配置" :body-style="{ padding: '32px 0' }" :bordered="false">
        <a-form @submit="handleSubmit" :form="form" :label-col="labelCol" :wrapper-col="wrapperCol">
            <a-form-item label="服务提供商" has-feedback>
                ...
                >
                    <a-select-option
                        ...
                        :key="index"
                        :value="item.dictKey"
                        @click="changeProvider(item.dictKey)"
                    >
                        {{ item.dictValue }}
                    </a-select-option>
                </a-select>
            ...
            <a-form-item label="模版ID" has-feedback>
                <a-input
                    placeholder="请输模版ID"
                    v-decorator="['templateId', { rules: [{ required: true, message: '请输入模版ID！' }] }]"
                />
            </a-form-item>
            ...
            <a-form-item label="regionId" has-feedback>
                <a-input
                    placeholder="请输入regionId"
                    v-decorator="[
                        'regionId',
                        { initialValue: 'cn-hangzhou', rules: [{ required: true, message: '请输入regionId！' }] },
                    ]"
                />
            </a-form-item>

            <a-form-item label="短信签名" has-feedback>
                <a-input
                    placeholder="请输入短信签名"
                    v-decorator="['signName', { rules: [{ required: true, message: '请输入短信签名！' }] }]"
                />
            </a-form-item>
...
// import { sysDictTypeDropDown } from '@/api/modular/system/dictManage';
    import { smsCheck, getOssSecret, saveSms, getSms } from '@/api/modular/system/setManage';
    export default {
        data() {
            return {
                providers: [],
        ...
        methods: {
             ...
             */
            dataTypeItem() {
                this.providers = this.$options.filters['dictData']('sms');
