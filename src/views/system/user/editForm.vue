                ...
                <a-col :md="12" :sm="24">
                    <a-form :form="form">
                        <a-form-item label="机构" :labelCol="labelCol" :wrapperCol="wrapperCol" has-feedback>
                            <a-tree-select
                                v-decorator="['empInfoReq.orgId', { rules: [{ required: true, message: '请选择机构！' }] }]"
                                style="width: 100%"
                        ...
                        <a-form :form="form">
                            <a-form-item v-show="false">
                                <a-input v-decorator="['empInfoReq.orgName']" />
                            </a-form-item>
                        </a-form>
                    </a-form>
                </a-col>
                <a-col :md="12" :sm="24">
                    <a-form :form="form">
                        <a-form-item label="工号" :labelCol="labelCol" :wrapperCol="wrapperCol" has-feedback>
                            <a-input placeholder="请输入工号" v-decorator="['empInfoReq.jobNum']" />
                        </a-form-item>
                    </a-form>
                </a-col>
            </a-row>
                    ...
                    <a-form :form="form">
                        <a-form-item label="职位信息" :labelCol="labelCol_JG" :wrapperCol="wrapperCol_JG" has-feedback>
                            <a-select
                                ...
                                style="width: 100%"
                                placeholder="请选择职位信息"
                                v-decorator="['empInfoReq.posIdList', { rules: [{ required: true, message: '请选择职位信息！' }] }]"
                            >
        ...
        methods: {
            ...
            getUserDetail(id) {
                sysUserDetail({ id: id }).then(res => {
                    const empInfo = res.data.empInfoRes;
                    const positions = [];
                    empInfo.positions.forEach(item => {
                        positions.push(item.posId + '');
                    });
                    this.form.getFieldDecorator('empInfoReq.orgName', { initialValue: empInfo.orgName });
                    this.form.getFieldDecorator('empInfoReq.posIdList', { initialValue: positions });
                    this.form.getFieldDecorator('empInfoReq.jobNum', { initialValue: empInfo.jobNum });
                    this.form.getFieldDecorator('empInfoReq.orgId', { initialValue: empInfo.orgId });
                    empInfo.extOrgPos.forEach(item => {
             ...
             */
            initrOrgName(value) {
                this.form.getFieldDecorator('empInfoReq.orgName', {
            ...
            handleSubmit() {
                ...
                validateFields((errors, values) => {
                    if (!errors) {
                        ...
                        values.empInfoReq['extIds'] = this.sysEmpParamExtList;
                        // eslint-disable-next-line eqeqeq
